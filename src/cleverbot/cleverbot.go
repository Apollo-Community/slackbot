...
func New() (*Session, error) {
	...
	// Need a jar to store cookies in
	...
	// Need to get a cookie from the homepage first, hence the cookie jar
	_, e = client.Get(PROTOCOL + HOST)
	if e != nil {
		return nil, e
	}

	return &Session{make([]string, 0), client, values}, nil
}
